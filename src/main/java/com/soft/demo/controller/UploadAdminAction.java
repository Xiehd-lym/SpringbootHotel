...
public class UploadAdminAction {
	...
	public String UploadImg(@RequestParam("upload") MultipartFile file,String num,
		...
		try {
			...
			String saveFile=Thread.currentThread().getContextClassLoader().getResource("").getPath().substring(0,Thread.currentThread().getContextClassLoader().getResource("").getPath().length()-16)+"/src/main/webapp/"+(savePath+"/"+file_name);
			saveFile = URLDecoder.decode(saveFile, "UTF-8");
			//文件类型限制
